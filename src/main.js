...
import { findActiveLine, parseLrc, visibleLineIndices } from "./lyrics.js";
import "./styles.css";
...
function renderLyricWindow(activeIndex) {
  ...
  elements.stack.replaceChildren();
  const visibleIndices = visibleLineIndices(state.lines.length, activeIndex, 5);
  ...
  for (const index of visibleIndices) {
    ...
    if (index === activeIndex) {
      ...
      text.textContent = state.lines[index].text;
      line.append(text);
    } else {
